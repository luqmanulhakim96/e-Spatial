      ...
      <!--Page Body part -->
            <div class="page-body p-4 text-dark">
                ...
                <div class="card rounded-lg" style="border-color: #003473 !important;">
                  <div class="card-header" style="text-align:center; border-color: #003473 !important; font-size: 130%; font-weight: bold;">Laporan Bilangan Keseluruhan Permohonan</div>

                  <div class="card-body">

                    <form class="" action="{{ route('laporan.laporan-1-tapis') }}" method="post">
                      @csrf
                      <div class="row pb-2">
                        <div class="col-md-1">
                        ...
                        <div class="col-md">
                          <select id="select_tarikh" class="custom-select  bg-light @error('select_tarikh') is-invalid @enderror" name="select_tarikh" value="{{ old('select_tarikh') }}" required>
                                ...
                                <option value="kelulusan" {{ old('select_tarikh') == "kelulusan" ? 'selected' : '' }}>Tarikh Kelulusan</option>
                          </select>
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-3">
                        ...
                        <a href="{{ route('laporan.laporan-1') }}" class="btn btn-primary m-auto"><i class="fa fa-refresh" aria-hidden="true"></i> Reset</a>
                      </div>
                    </div>
                      ...
                      <table class="table table-striped table-bordered" id="table-laporan" style="width: 100%;">
                        <thead>
                            <tr>
                              ...
                              <th class="all">KELULUSAN PERMOHONAN</th>
                              <th class="all">TARIKH KELULUSAN</th>
                            </tr>
                        </thead>
                        <tbody>
                          @foreach($listPermohonan as $laporan1)
                          <tr>
                            <td></td>
                            <td>
                              <div style="padding : 4px;"></div>
                              @if($laporan1->tarikh_permohonan != null)
                              {{ Carbon\Carbon::parse($laporan1->tarikh_permohonan)->format('d-m-Y ') }}
                              @else
                              -
                              @endif
                            </td>
                            ...
                            <td>
                              <div style="padding : 4px;"></div>
                              @if($laporan1->user->nama_kementerian != null)
                              {{$laporan1->user->nama_kementerian}}
                              @else
                              -
                              @endif
                            </td>
                            <td>
                              ...
                              <span class="badge m-1 p-2 badge-pill badge-success" style="font-size: 11px;">{{$laporan1->status_permohonan}}</span>
                              @elseif( $laporan1->status_permohonan == 'Sedang Diproses')
                              <span class="badge m-1 p-2 badge-pill badge-warning" style="font-size: 11px;">{{$laporan1->status_permohonan}}</span>
                              @else
                              <span class="badge m-1 p-2 badge-pill badge-danger" style="font-size: 11px;">{{$laporan1->status_permohonan}}</span>
                              @endif
                            </td>
                            ...
                            <td>
                              <div style="padding : 4px;"></div>
                              @if($laporan1->tarikh_ketua_pengarah != null)
                              {{ Carbon\Carbon::parse($laporan1->tarikh_ketua_pengarah)->format('d-m-Y ') }}
        ...
        var t = $(tablelaporan).DataTable({
          ...
          "buttons": [
              'excel',
              {
                  ...
                  pageSize: 'A4',
                  title: 'Laporan Bilangan Keseluruhan Permohonan',
              },
              {
                  extend: 'print',
                  text: 'Cetak',
